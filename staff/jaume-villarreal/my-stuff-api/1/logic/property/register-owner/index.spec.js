...
describe('logic', () => {
    ...
    describe('add a new owner on an existing property', () => {
        ...
        it('should add a new owner on correct data', async() => {
            await logic.registerNewPropertyOwner(propertyId , userId1 , userId2)

            const property = await Property.findById(propertyId)
                expect(property).to.exist
                expect(property.owners.length).to.equal(2)
                expect(property.owners[0].toString()).to.equal(userId1)
                expect(property.owners[1].toString()).to.equal(userId2)
        })
        ...
        it('should fail on unexisting new owner' , async ()=>{
            userId2 = '5d5d5530531d455f75da9fF9'
            try{
                await logic.registerNewPropertyOwner(propertyId , userId1 , userId2)
            }catch({ message }){
                expect(message).to.equal('user with id 5d5d5530531d455f75da9fF9 does not exist')
        ...
        it('should fail if new owner already reigstered as owner' , async ()=>{
            const newProperty = await new Property({address , m2 , year , cadastre})
            const propId = newProperty.id
            newProperty.owners.push(userId1)
            newProperty.owners.push(userId2)

            await newProperty.save()
            
            try{
                await logic.registerNewPropertyOwner(propId , userId1 , userId2)
            }catch({ message }){
                expect(message).to.equal(`user with id ${userId2} is already owner of property with id ${propId}`)
            }
        })
